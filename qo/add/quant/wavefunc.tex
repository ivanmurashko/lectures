...
\begin{definition}[Чистое состояние]
...
\begin{definition}[Смешанное состояние]
Если состояние системы описывается матрицей плотности $\hat{\rho}$
...
называется декогеренцией.\rindex{Декогеренция}
...
Адамара по отношению к исходному \rindex{Преобразование Адамара}:
\begin{eqnarray}
...
\mbox{ с. }$ \rindex{Декогеренция!скорость}

%% В качесте примера можно рассмотреть так
%% называемого кота Шредингера, который может находится в двух состояниях
%% $\ket{L}$ - кот жив и $\ket{D}$ - кот мертв. 
%% Вместе с этими двумя состояниями согласно принципу суперпозиции 
%% вполне допустимо состояние $\left|\psi\right> = C_L \ket{L} + C_D
%% \ket{D}$ - суперпозиции живого и мертвого кота. Этот пример
...
%% $\left|C_L\right|^2$ он находится в состоянии $\ket{L}$ и с
%% вероятностью $\left|C_D\right|^2$ - в состоянии
%% $\ket{D}$. Такие системы называются смешанными. Очевидно что
...
Рассмотрим вначале чистые состояния \rindex{Чистое состояние}
...
\begin{example}[Измерение энергии двухуровневого атома]
...
измерения. Смешанные состояния \rindex{Смешанное состояние}
