...
C_{a, n}\left(t\right),
  \nonumber
\end{eqnarray}
где $\delta = \frac{\omega - \omega_{ab}}{2}$ - расстройка частоты.
...
\begin{eqnarray}
  \ddot{x} =  -i \omega_R \frac{d \left(e^{2 i \delta t} 
    C_{a, n}\left(t\right)\right) }{d t} =
  \nonumber \\
  = i \delta \left(- i \omega_R e^{2 i\delta t} 
  C_{a, n}\left(t\right) \right) -i \omega_R e^{2 i \delta t}
  \frac{d C_{a, n}\left(t\right)}{dt} =
  \nonumber \\
  = 2 i \delta \dot{x} - \omega_R^2 e^{-2 i \delta t} e^{2 i \delta t} x =
  \nonumber \\
  = 2 i \delta \dot{x} - \omega_R^2 x,
  \nonumber
\end{eqnarray}
т. е.
\begin{equation}
  \ddot{x} -2 i \delta \dot{x} + \omega_R^2 x = 0.
...
\begin{eqnarray}
  ...
  \nonumber
\end{eqnarray}
%% (%i1) eq: 'diff(x(t),t,2) - 2*%i*d*'diff(x(t),t) + w^2*x(t) = 0;
%% (%i2) atvalue(x(t),t=0,0);
%% (%i3) atvalue('diff(x(t),t),t=0,-%i*w);
%% (%i4) desolve(eq, x(t));
%% positive;
%% (%i5) 
Результат:
\begin{equation}
  x\left(t\right) = i e^{ i \delta t}
  \frac{\omega_R}{\sqrt{\omega_R^2 + \delta^2}}
  \sin{\left(\sqrt{\omega_R^2 + \delta^2}t\right)},
  \nonumber
\end{equation}
